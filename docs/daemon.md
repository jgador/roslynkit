...
- A compatibility identity includes the current user, canonical Git worktree and target, process architecture, resolved MSBuild identity, applicable `global.json`, agreed build environment values, and the runtime directory used by local IPC.
- The endpoint is a fixed-length opaque hash of the canonical identity. Repository paths never appear in endpoint names.
- A short-lived `DaemonBootstrapLease` prevents concurrent clients from racing to start a server. After acquiring it, the client rechecks connectivity before spawning and holds it only until the handshake reports readiness. The separate `DaemonLifetimeLease` enforces one live server for the identity. Both use cross-process mutexes held by dedicated owner threads; Windows uses the global mutex namespace so desktop sessions for the same identity cannot create separate owners.
- Readiness requires a protocol handshake. The existence of a pipe or socket is not sufficient.
...
6. Queries `MSBuildLocator` with the target directory to record the selected instance, discovery type, and MSBuild path.
7. Captures the process architecture.
...
`index --text-only` and `search --text-only` are explicit daemon bypasses for search-only constrained hosts. They construct one in-process `AdhocWorkspace` from repository C# files, use a separate text-only target partition, and never attempt daemon transport or `MSBuildWorkspace` loading. `--project` is unavailable in that synthetic single-project view. `--compact` can remove navigation metadata for an LLM judgment step, while `--balanced` reserves half of a bounded match set for tests when both production and test paths are available.
...
- The command's absolute UTC deadline and the connection-lifetime cancellation token both flow into `WorkspaceDaemonSession.ExecuteAsync`. `WorkspaceDaemonServer` cancels that token when the peer disconnects; cancellation of the short-lived client closes the same request path. `Program.Main` creates a process-lifetime cancellation token and passes it through both hidden-daemon and ordinary command routing. On Unix, `SIGHUP`, `SIGINT`, and `SIGTERM` request that cancellation; on Windows, `Console.CancelKeyPress` does the same.
...
- The server bounds live connections at 32. Every connection must complete a handshake and may then issue exactly one command, status, or stop operation. A handshake-only connection may close cleanly after readiness is confirmed.
...
- A clean peer disconnect, pipe failure, or unexpected byte after a command frame cancels the connection token passed into the lifecycle host. Malformed clients close only their connection; the listener remains available.
- Stop writes its acknowledgement before canceling the accept loop, then relies on the lifecycle host to drain or cancel existing work. When `WorkspaceDaemonServer.RunAsync` receives process-lifetime cancellation, it disposes the host before awaiting connection handlers, so active Roslyn work cannot keep that shutdown path alive indefinitely. A hidden daemon receiving Unix `SIGHUP`, `SIGINT`, or `SIGTERM` suppresses the default signal action, follows that cancellation path, and exits with status `130` after cleanup.
