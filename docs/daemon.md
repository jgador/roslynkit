...
Search indexing supports projects with one target framework and rejects multi-targeted projects rather than selecting a framework implicitly. Every indexed project and source document must have an existing physical path inside the target's Git worktree; missing paths, external projects, and external linked files are rejected. Search covers all target projects by default, can narrow to `--project`, and indexes non-generated declarations only, excluding source-generated declarations and generated source paths below `bin` or `obj`. Search returns ranked declarations for agent-mediated navigation through existing `id:` and `loc:` values; it has no standard-input pipeline contract.
