...
- [.agents/skills/benchmark/SKILL.md](../../.agents/skills/benchmark/SKILL.md) is a standalone explicit-only workflow skill for the native benchmark harness. The harness owns case selection, retrieval, JSON Lines (JSONL) accounting, evidence validation, and reports; keep the skill concise and route-oriented. Keep it outside the stable RoslynKit bundle so `roslynkit init` does not scaffold it.
