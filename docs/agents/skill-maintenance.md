...
- [docs/dotnet-tool-release.md](../dotnet-tool-release.md) owns release preparation, the grouped manual Bash checklist, explicit global replacement, and the separate manual NuGet.org upload. There is no release workflow skill. [scripts/prepare-roslynkit-package.ps1](../../scripts/prepare-roslynkit-package.ps1) owns the folder-feed build, [scripts/RoslynKit.Packaging.ps1](../../scripts/RoslynKit.Packaging.ps1) owns shared isolated installation and package-hash validation, and [scripts/test-roslynkit-commands.ps1](../../scripts/test-roslynkit-commands.ps1) owns automated command coverage. [scripts/test-roslynkit-package.ps1](../../scripts/test-roslynkit-package.ps1) tests the isolated installation, while [scripts/install-roslynkit-global.ps1](../../scripts/install-roslynkit-global.ps1) and [scripts/test-roslynkit-global.ps1](../../scripts/test-roslynkit-global.ps1) own explicit global replacement and verification. Update the guide's manual examples alongside changes to runtime commands or fixture coordinates; keep public upload separate from local preparation.
