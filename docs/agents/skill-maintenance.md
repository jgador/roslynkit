...
- [.agents/skills/benchmark/SKILL.md](../../.agents/skills/benchmark/SKILL.md) is a standalone repo workflow skill for explicit direct-worktree Codex benchmark requests. Its controller workflow owns benchmark docs, case data, preflight, and review; its measured-session behavior permits the benchmark skill in both conditions and the stable RoslynKit skill plus its command and output references only in the RoslynKit condition. Keep it outside the stable RoslynKit bundle so `roslynkit init` does not scaffold it.
