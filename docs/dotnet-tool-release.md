...
1. Set the new `<Version>` in `Directory.Build.props` using a bare NuGet version such as `0.1.0` or a prerelease such as `0.1.1-dev.1`. Use the leading `v` only for Git tags or release titles such as `v0.1.0`.
...
Use a prerelease `<Version>` such as `0.1.1-dev.1` and run the dev installer from the current checkout:
