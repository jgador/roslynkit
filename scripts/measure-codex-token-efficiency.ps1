...
function Get-ForbiddenExternalCommandViolations {
    ...
    $atlasPattern = '(?i)(\.codex\\atlas(\\|$|[\s"`''])|(^|[\s;&|"`''])(atlas-csharp-mapper|atlas-doc-mapper|atlas-test-mapper)([\s;&|"`'']|$))'
...
function Test-ForbiddenExternalToolName {
    ...
    return $Name -match '(?i)(^|[._:-])(scout|explorer|worker)($|[._:-])|atlas-csharp-mapper|atlas-doc-mapper|atlas-test-mapper' -or
        ...
        foreach ($record in Read-JsonLines -Path $path) {
            if ($record.type -eq "response_item" -and
                ...
                if (@("shell_command", "exec_command", "shell") -notcontains $toolName) {
                    ...
                    $repoLocalMemoryToolName = 'syn' + 'apse'
                    if ($arguments -match '(?i)\b(scout|explorer|worker|atlas-csharp-mapper|atlas-doc-mapper|atlas-test-mapper)\b' -or
...
- Do not use Atlas files or tools: .codex\atlas, atlas-csharp-mapper, atlas-doc-mapper, atlas-test-mapper, or Atlas scripts.
