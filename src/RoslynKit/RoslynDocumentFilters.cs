...
public static class RoslynDocumentFilters
{
    ...
    /// <summary>
    /// Detects generated source injected from an extracted NuGet package into an MSBuild project.
    /// </summary>
    public static async Task<bool> IsPackageInjectedGeneratedSourceAsync(
        Document document,
        CancellationToken cancellationToken)
    {
        var fullPath = NormalizePath(document.FilePath);
        if (fullPath is null
            || !File.Exists(fullPath)
            || !HasNuGetPackageMetadataAncestor(fullPath))
        {
            return false;
        }
    ...
    private static bool HasNuGetPackageMetadataAncestor(string fullPath)
    {
        for (var directory = Directory.GetParent(fullPath); directory is not null; directory = directory.Parent)
        {
            if (File.Exists(Path.Combine(directory.FullName, ".nupkg.metadata")))
            {
                return true;
            }
        }
