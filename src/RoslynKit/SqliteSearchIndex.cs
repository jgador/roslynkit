...
internal sealed class SqliteSearchIndex
{
    ...
    /// <summary>
    /// Replaces all searchable symbols for one target in a single transaction.
    /// </summary>
    ...
    /// <summary>
    /// Replaces selected project partitions while preserving other targets and projects.
    /// </summary>
    ...
    private async Task<SqliteConnection?> TryOpenReadConnectionAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var version = await ReadSchemaVersionAsync(connection, null, cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            await ExecuteNonQueryAsync(connection, transaction, """
                DROP TABLE IF EXISTS semantic_catalog_operation_cache;
                DROP TABLE IF EXISTS semantic_catalog_relations;
                DROP TABLE IF EXISTS semantic_catalog_symbols;
                DROP TABLE IF EXISTS semantic_catalog_projects;
                DROP TABLE IF EXISTS search_index_fts;
                DROP TABLE IF EXISTS search_index_symbols;
                DROP TABLE IF EXISTS search_index_targets;
                DROP TABLE IF EXISTS search_index_schema;
                """, cancellationToken);
            await EnsureSchemaAsync(connection, transaction, cancellationToken);
            await ExecuteNonQueryAsync(connection, transaction, $"PRAGMA user_version = {SchemaVersion};", cancellationToken);
        }
        else
        {
            await ValidateSearchSchemaAsync(connection, transaction, cancellationToken);
    ...
    private static void ValidateSchemaVersion(int version)
    {
        if (version > SchemaVersion)
        {
            throw new InvalidOperationException(
                $"The SQLite search index schema version {version} is newer than supported version {SchemaVersion}.");
        }
    }
