...
The search index accepts only projects with one target framework. It rejects multi-targeted projects instead of selecting a framework implicitly. Every indexed project and source document must have an existing physical path inside the target's Git worktree; missing paths, external projects, and external linked files are rejected. It indexes non-generated declarations only; source-generated declarations and generated source paths below `bin` or `obj` are excluded. By default a target search covers every project; use `--project` to narrow it, `--kind` to select symbol kinds, and `--max-results` to change the default limit of 20.
