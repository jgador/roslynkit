...
When an English-oriented question describes a C# responsibility but no declaration name is known, follow the intent-based symbol discovery workflow in [.agents/skills/roslynkit-dev/SKILL.md](.agents/skills/roslynkit-dev/SKILL.md). In this repository, omit `--target` from ordinary RoslynKit commands and let RoslynKit infer the Git repository project forest and the `.roslynkit/roslynkit.db` catalog. The presence of [RoslynKit.slnx](RoslynKit.slnx) or a known `.csproj` path does not by itself justify an override. Add `--target` only when the requested evidence must be restricted to a particular solution, solution filter, project, or repository directory, and pass `--index-path` only for explicit override testing. `search` creates, validates, and refreshes the index automatically. Use `index` only to prepare it deliberately, and use `--rebuild` only when the selected target partition must be recreated. See [.agents/skills/roslynkit/references/commands.md](.agents/skills/roslynkit/references/commands.md) for exact runtime syntax and options.
