...
public sealed class RoslynSearchCorpusBuilderTests
{
    ...
    [Fact]
    public async Task BuildAsync_SkipsPackageInjectedGeneratedDocumentAndRejectsOtherInvalidDocumentPaths()
    {
        using var loaded = await RoslynWorkspaceLoader.LoadAsync(TestPaths.FixtureProjectPath(), TestContext.Current.CancellationToken);
        var project = Assert.Single(loaded.Solution.Projects);
        var temporaryRoot = Path.Combine(Path.GetTempPath(), "roslynkit-tests", Guid.NewGuid().ToString("N"));
        var packageVersionDirectory = Path.Combine(temporaryRoot, "packages", "package.injected", "1.2.3");
        var packageContentDirectory = Path.Combine(packageVersionDirectory, "contentFiles", "cs", "any");
        var packageGeneratedDocumentPath = Path.Combine(packageContentDirectory, "PackageGenerated.cs");
        var packageNonGeneratedDocumentPath = Path.Combine(packageContentDirectory, "PackageNonGenerated.cs");
        var userLinkedGeneratedDocumentPath = Path.Combine(temporaryRoot, "linked", "UserLinkedGenerated.g.cs");
        ...
        try
        {
            Directory.CreateDirectory(packageContentDirectory);
            Directory.CreateDirectory(Path.GetDirectoryName(userLinkedGeneratedDocumentPath)!);
            await File.WriteAllTextAsync(
                Path.Combine(packageVersionDirectory, ".nupkg.metadata"),
                "{}",
                TestContext.Current.CancellationToken);
            await File.WriteAllTextAsync(
                packageGeneratedDocumentPath,
                "// <auto-generated>\nnamespace PackageGeneratedSource; public sealed class PackageGeneratedDocumentSymbol { }",
                TestContext.Current.CancellationToken);
            await File.WriteAllTextAsync(
                packageNonGeneratedDocumentPath,
                "namespace PackageSource; public sealed class PackageNonGeneratedDocumentSymbol { }",
                TestContext.Current.CancellationToken);
            await File.WriteAllTextAsync(
                userLinkedGeneratedDocumentPath,
                "// <auto-generated>\nnamespace UserLinkedSource; public sealed class UserLinkedGeneratedDocumentSymbol { }",
                TestContext.Current.CancellationToken);
            ...
            var solution = loaded.Solution
                .AddDocument(
                    ...
                    "PackageGenerated.cs",
                    "// <auto-generated>\nnamespace PackageGeneratedSource; public sealed class PackageGeneratedDocumentSymbol { }",
                    filePath: packageGeneratedDocumentPath)
                .AddDocument(
                    DocumentId.CreateNewId(project.Id),
                    "PackageNonGenerated.cs",
                    "namespace PackageSource; public sealed class PackageNonGeneratedDocumentSymbol { }",
                    filePath: packageNonGeneratedDocumentPath)
                .AddDocument(
                    DocumentId.CreateNewId(project.Id),
                    "UserLinkedGenerated.g.cs",
                    "// <auto-generated>\nnamespace UserLinkedSource; public sealed class UserLinkedGeneratedDocumentSymbol { }",
                    filePath: userLinkedGeneratedDocumentPath)
                .AddDocument(
                    DocumentId.CreateNewId(project.Id),
                    "MissingSource.cs",
            ...
            Assert.All(result.Issues, issue => Assert.Equal("document-path-invalid", issue.Code));
            Assert.DoesNotContain(result.Issues, issue => issue.Message.Contains("PackageGenerated.cs", StringComparison.Ordinal));
            Assert.Contains(result.Issues, issue => issue.Message.Contains("PackageNonGenerated.cs", StringComparison.Ordinal)
                && issue.Message.Contains("outside", StringComparison.OrdinalIgnoreCase));
            Assert.Contains(result.Issues, issue => issue.Message.Contains("UserLinkedGenerated.g.cs", StringComparison.Ordinal)
                && issue.Message.Contains("outside", StringComparison.OrdinalIgnoreCase));
            Assert.Contains(result.Issues, issue => issue.Message.Contains("MissingSource.cs", StringComparison.Ordinal)
                && issue.Message.Contains("physical", StringComparison.OrdinalIgnoreCase));
            Assert.DoesNotContain(result.Records, record => record.Name == "PackageGeneratedDocumentSymbol");
            Assert.DoesNotContain(result.Records, record => record.Name == "PackageNonGeneratedDocumentSymbol");
            Assert.DoesNotContain(result.Records, record => record.Name == "UserLinkedGeneratedDocumentSymbol");
