...
- `DaemonCommandExecutor` handles `daemon status` and `daemon stop` without loading a workspace or starting a daemon; until public control routing is implemented, both report `state: not-running`.
- `GitWorkspaceIdentityResolver` validates the committed Git worktree boundary and captures the target, `global.json`, SDK/MSBuild, build environment, RoslynKit build, protocol, and architecture compatibility inputs. `DaemonServerRunner` consumes it for hidden-process construction; public CLI client routing remains absent.
