...
- TypeScript/JavaScript fixtures: `tests/TypeScriptFixture/`, `tests/JavaScriptFixture/`
- Native-preview bridge: `src/RoslynKit/TypeScriptBridge/`
...
- `GitWorkspaceIdentityResolver` validates the committed Git worktree boundary and captures the target, `global.json`, software development kit (SDK)/MSBuild, build environment, RoslynKit build, protocol, architecture, and optional TypeScript runtime compatibility inputs. Both `DaemonClient` and `DaemonServerRunner` consume the same resolution before deriving the endpoint.
...
- `WorkspaceDaemonSession` owns disposable workspace generations, reconciles fingerprints before leasing an immutable snapshot, drains active readers before refresh, admits at most three clean readers, gives pending reloads priority, and performs the single quiet-period retry. C# generations own `RoslynWorkspaceLoader`; TypeScript generations share one session-owned Node bridge, native API, and compiler process while replacing and disposing snapshots. The hidden server owns the session through `WorkspaceDaemonHost`; public workspace commands reach it through `DaemonClient`.
...
- `TypeScriptBridgeClient` owns the JSON-lines Node process. `TypeScriptWorkspaceCommandBackend` routes native semantic commands; `TypeScriptDaemonBackendOwner` preserves the bridge and API across daemon generations; `TypeScriptRuntimeResolver` discovers Node, the packaged bridge, and native-preview. The bridge imports `@typescript/native-preview/unstable/sync` and its native AST surface, owns the project and current snapshot, and emits deterministic `ts:` selectors.
- `SearchCommandService` and `TypeScriptSearchCommandService` coordinate `index` and `search` around an explicit repository-local SQLite index path. The version-2 database stores language-marked target partitions and symbols while migrating version-1 C# indexes in place. It retains repository-relative identities, full-text search with internal Best Matching 25 ranking, and write-ahead logging; public locations are reconstructed from the Git root.
...
flowchart TD
    ...
    R --> BK{"Workspace target backend"}
    BK -->|C#| J2["RoslynCommandExecutor.ExecuteAsync<br/>caller-owned workspace overload"]
    BK -->|tsconfig / jsconfig| TS["TypeScriptWorkspaceCommandBackend<br/>retained Node + API + snapshot"]
    J2 --> K{"Command dispatch switch"}
    TS --> TSO["native-preview semantics<br/>deterministic markdown"]
    TSO --> PR
...
- TypeScript/JavaScript backend: [WorkspaceTarget.cs](../../src/RoslynKit/WorkspaceTarget.cs), [WorkspaceCommandBackend.cs](../../src/RoslynKit/WorkspaceCommandBackend.cs), [TypeScriptRuntimeResolver.cs](../../src/RoslynKit/TypeScriptRuntimeResolver.cs), [TypeScriptBridgeClient.cs](../../src/RoslynKit/TypeScriptBridgeClient.cs), [TypeScriptSearchCommandService.cs](../../src/RoslynKit/TypeScriptSearchCommandService.cs), and [TypeScriptBridge/bridge.mjs](../../src/RoslynKit/TypeScriptBridge/bridge.mjs); start symbols `WorkspaceTarget.Resolve`, `TypeScriptWorkspaceCommandBackend.CreateAsync`, `TypeScriptDaemonBackendOwner.LoadGenerationAsync`, `TypeScriptBridgeClient.StartAsync`, and `NativePreviewWorkspace.execute`; tests [TypeScriptBackendTests.cs](../../tests/RoslynKit.Tests/TypeScriptBackendTests.cs) plus `src/RoslynKit/TypeScriptBridge/test/`.
- Intent-based symbol search: [SearchCommandService.cs](../../src/RoslynKit/SearchCommandService.cs), [TypeScriptSearchCommandService.cs](../../src/RoslynKit/TypeScriptSearchCommandService.cs), [SearchQueryTokenizer.cs](../../src/RoslynKit/SearchQueryTokenizer.cs), [SearchIndexPathPolicy.cs](../../src/RoslynKit/SearchIndexPathPolicy.cs), [RoslynSearchCorpusBuilder.cs](../../src/RoslynKit/RoslynSearchCorpusBuilder.cs), [SqliteSearchIndex.cs](../../src/RoslynKit/SqliteSearchIndex.cs), [SqliteSearchIndexModels.cs](../../src/RoslynKit/SqliteSearchIndexModels.cs), and [SqliteSearchIndexTypeMaps.cs](../../src/RoslynKit/SqliteSearchIndexTypeMaps.cs). C# uses Git-aware project refresh planning; TypeScript builds an atomic native-snapshot corpus. Dapper uses strict explicit column maps, while Microsoft.Data.Sqlite owns provider connections, writer transactions, migration, and schema operations.
...
- TypeScript native semantics, process reuse, snapshot refresh, search selectors, and prerequisites -> [TypeScriptBackendTests.cs](../../tests/RoslynKit.Tests/TypeScriptBackendTests.cs) and `src/RoslynKit/TypeScriptBridge/test/`
...
- Bridge install/test: `npm ci --prefix .\src\RoslynKit\TypeScriptBridge` then `npm test --prefix .\src\RoslynKit\TypeScriptBridge`
...
- Use `roslynkit-dev` for repo-local C#, TypeScript, or JavaScript semantic inspection unless the task is explicitly about the stable global tool.
