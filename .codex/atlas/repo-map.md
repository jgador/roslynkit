...
- Codex benchmarking: [scripts/benchmark-codex.ps1](../../scripts/benchmark-codex.ps1), [benchmarks/codex-cases.json](../../benchmarks/codex-cases.json), [docs/benchmark-codex.md](../../docs/benchmark-codex.md), and [.agents/skills/benchmark/SKILL.md](../../.agents/skills/benchmark/SKILL.md). The opt-in controller runs every child with the repository root as `--cd`, prepares one shared repository-local `artifacts/roslynkit.db` index once before all cases and trials, and records a content-manifest baseline that permits initial dirty state while detecting later worktree-content changes. It resolves global `rg` and `roslynkit` from the fully inherited workstation environment, retains the unmeasured full-access `rg --version; roslynkit --version` preflight, requires both conditions to read the benchmark skill, permits only the RoslynKit condition to read the stable RoslynKit skill plus its command and output references and invoke RoslynKit, requires a two-minute shell tool timeout plus serial bounded searches and opaque symbol IDs for RoslynKit, stops workspace daemons between sessions, uses the active workstation Codex home and configuration while retaining benchmark command-line overrides, and reads token accounting only from current-run JSON Lines events. Ordinary invalid measured sessions remain in the reports while later scheduled sessions continue without retry; preparation failures, preflight failures, and repository content changes stop the controller. Measured sessions may not inspect AGENTS, Atlas, the runner, benchmark docs or data, private criteria, prior artifacts, or other skills. This direct-worktree workflow has reduced isolation.
