...
- Native benchmarking: [tests/Integration/Benchmarking/](../../tests/Integration/Benchmarking/), [tests/Integration/Benchmarking/cases.json](../../tests/Integration/Benchmarking/cases.json), [docs/benchmark.md](../../docs/benchmark.md), and [.agents/skills/benchmark/SKILL.md](../../.agents/skills/benchmark/SKILL.md). The explicit-only .NET harness compares bounded raw-text retrieval with direct RoslynKit apphost `--text-only --compact --balanced` retrieval. It builds the Release apphost by default, bypasses daemon startup, and supplies only retrieval text to ephemeral tool-free Codex judges. Judges inherit the active host's `CODEX_HOME` user configuration and provider while clearing `CODEX_THREAD_ID` and ignoring project instructions and command rules. Typed JSON Lines (JSONL) accounting, evidence groups, and a strict per-pair 20% input-token threshold determine comparability. One schema-versioned `run.json` beneath `artifacts/benchmark/` is the report source for `runs.csv` and `summary.md`; resume and report-only modes hydrate that document.
...
- Benchmark dry run: `dotnet run --project ./tests/Integration/Benchmarking/RoslynKit.Benchmarking.csproj -- --dry-run --trials 1`
