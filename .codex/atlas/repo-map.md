...
- Codex benchmarking: [scripts/benchmark-codex.ps1](../../scripts/benchmark-codex.ps1), [benchmarks/codex-cases.json](../../benchmarks/codex-cases.json), [docs/benchmark-codex.md](../../docs/benchmark-codex.md), and [.agents/skills/benchmark/SKILL.md](../../.agents/skills/benchmark/SKILL.md). The opt-in controller keeps prompts and review criteria outside separate one-commit raw and RoslynKit snapshots, restores both snapshots before timing, gives only the RoslynKit snapshot a prepared `artifacts/roslynkit.db` index, retains both snapshots by default and always reports their temporary locations, runs children with the snapshot as `--cd`, resolves global `rg` and `roslynkit` from the fully inherited workstation environment, checks both tools from recorded command output in one unmeasured full-access preflight, requires serial bounded RoslynKit searches and rejects observable command overlap, stops workspace daemons between sessions, uses the active workstation Codex home and configuration while retaining benchmark command-line overrides, and reads token accounting only from current-run JSON Lines events.
