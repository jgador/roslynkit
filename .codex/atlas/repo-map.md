...
- Continuous integration: [.github/workflows/ci.yml](../../.github/workflows/ci.yml) validates Ubuntu and Windows builds, tests, Bash benchmark regression, and PowerShell packaging portability checks.
...
- `SearchCommandService` coordinates the `index` and `search` commands around an explicit repository-local SQLite index path. The database stores separate target partitions, repository-relative target identities, project paths, and declaration source paths, full-text search with internal Best Matching 25 ranking, and write-ahead logging; public target and declaration locations are reconstructed as absolute paths from the resolved repository root. Search preserves declaration-kind navigation priority, then ranks broader query-term coverage and Best Matching 25 relevance before the production-versus-test path tie-breaker so compact results can surface focused evidence. It is independent of daemon endpoint identity. `search` validates and incrementally refreshes a target automatically; `index` is the strict explicit refresh boundary and `--rebuild` forces that target's full rebuild. A prior coherent partition may answer concurrent searches as `stale` while a refresh runs.
...
- Codex benchmarking: [scripts/benchmark-codex.sh](../../scripts/benchmark-codex.sh), [scripts/benchmark_codex_support.py](../../scripts/benchmark_codex_support.py), [benchmarks/codex-cases.json](../../benchmarks/codex-cases.json), [docs/benchmark-codex.md](../../docs/benchmark-codex.md), and [.agents/skills/benchmark/SKILL.md](../../.agents/skills/benchmark/SKILL.md). The opt-in Bash controller uses GNU-style long options, requires Python 3.10 or later only for its standard-library helper, runs on Git Bash for Windows plus Windows Subsystem for Linux (WSL), Linux, and macOS, and runs every child with the repository root as `--cd`. It disables `unified_exec`, prepares one shared repository-local index below `artifacts/` before all cases and trials, and records a content-manifest baseline that permits initial dirty state while detecting later worktree-content changes. An optional isolated RoslynKit executable path must name `roslynkit` or `roslynkit.exe`; the controller prepends its parent directory to measured child `PATH` without changing the host-global installation. The hidden `--internal-tool-probe-path` mode runs through Bash, resolves and executes `rg` and `roslynkit` from inherited `PATH`, and writes `preflight/tool-probe.json` with host kind, resolved paths, outputs, and individual exit codes; the controller requires one successful child command event and validates that artifact without using command `aggregated_output`. Generated measured commands use Bash. Compliance recursively normalizes Bash, `sh`, and `zsh` `-c` or `-lc` plus GNU `timeout` envelopes for required reads, RoslynKit invocation, ordering, overlap, the hard eight-invocation ceiling, and repository-root recursive-search rejection while retaining quoted-text negatives; PowerShell and `cmd` wrappers are noncompliant. Reports distinguish RoslynKit invocations from total tool calls. Both conditions must read the benchmark skill through `bash -lc` and `cat`; only the RoslynKit condition may read the stable RoslynKit skill and references or invoke RoslynKit. Both conditions limit recursive and literal searches to explicit permitted source or test paths. RoslynKit uses a two-minute shell timeout, an adaptive search sequence capped at 10 results for the initial and refined queries and 20 or at most 50 for one final fallback, serial invocations, opaque symbol IDs, and daemon cleanup between sessions. Case prompts explicitly request every behavior dimension scored by their private rubrics; rubrics retain expected facts, implementation surfaces, and focused tests without adding hidden task requirements. Strict token accounting comes only from the current run's terminal JSON Lines usage event and records cached input, cache writes, regular uncached input, output, and reasoning output. Reports project GPT-5.6 Sol, Terra, and Luna Standard application programming interface costs, but mark 272K request-threshold metrics unknown because `codex exec --json` exposes a completed-turn aggregate rather than underlying request sizes. Operational validity stays separate from structured manual correctness review; cost and token savings use only reviewed-correct runs. Timing comparisons are valid only between conditions in the same host-local run; pre-normalization runs and different hosts are not timing-comparable. Ordinary invalid measured sessions remain in reports while later sessions continue without retry; preparation failures, preflight failures, and repository content changes stop the controller. Measured sessions may not inspect AGENTS, Atlas, the runner, benchmark docs or data, private criteria, prior artifacts, or other skills. This direct-worktree workflow has reduced isolation.
...
- Bash benchmark regression -> [tests/bash/benchmark-regression-tests.sh](../../tests/bash/benchmark-regression-tests.sh)
