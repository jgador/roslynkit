...
Use `& $roslynkitDev help <command>` for exact runtime syntax and options. In the RoslynKit repository, [docs/agents/roslynkit-command-reference.md](../../../docs/agents/roslynkit-command-reference.md) is the generated checked-in command reference.
...
- If the current `.cs` file and cursor are already known, start with a position-based command on that location before you reach for `symbols`.
